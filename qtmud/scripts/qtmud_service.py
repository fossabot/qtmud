...
@click.option('--coloredOutput/--no-coloredOutput', default=True, help='Whether to allow output to be colorized.  If true, qtMUD expects the `termcolor` module to be importable.')
def load_start_run(conf, datadir, mudlib, verbose, quiet, coloredoutput):
    try:
        qtmud = importlib.import_module('qtmud')
        qtmud.log.info('Imported qtMUD version {}'.format(qtmud.__version__))
    ...
    DATA_DIR = datadir
    try:
        qtmud.load(mudlib_name=mudlib, features={'coloredOutput': coloredoutput})
        try:
            qtmud.start()
            try:
                qtmud.run()
